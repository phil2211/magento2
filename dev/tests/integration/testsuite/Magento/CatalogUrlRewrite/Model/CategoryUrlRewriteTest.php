...
class CategoryUrlRewriteTest extends TestCase
{
     ...
     * @return void
     */
    public function testAncestorRewriteGenerationPreservesCustomRewriteForDescendantPath(): void
    {
        $storeId = 1;
        $descendantCategoryId = 402;
        $categoryUrlPathGenerator = $this->objectManager->get(CategoryUrlPathGenerator::class);
        $descendantCategory = $this->categoryRepository->get($descendantCategoryId, $storeId);
        $requestPath = $categoryUrlPathGenerator->getUrlPathWithSuffix($descendantCategory, $storeId);
        $urlPersist = $this->objectManager->get(UrlPersistInterface::class);
        $urlPersist->deleteByData(
            [
                UrlRewrite::ENTITY_TYPE => CategoryUrlRewriteGenerator::ENTITY_TYPE,
                UrlRewrite::ENTITY_ID => $descendantCategoryId,
                UrlRewrite::REQUEST_PATH => $requestPath,
                UrlRewrite::STORE_ID => $storeId,
            ]
        );

        $customRewrite = $this->objectManager->get(UrlRewriteFactory::class)->create()
            ->setEntityType('custom')
            ->setEntityId(0)
            ->setRequestPath($requestPath)
            ->setTargetPath('about-us')
            ->setRedirectType(0)
            ->setStoreId($storeId)
            ->setIsAutogenerated(0);
        $urlPersist->replace([$customRewrite]);

        $ancestorCategory = $this->categoryRepository->get(400, $storeId);
        $categoryUrlRewriteGenerator = $this->objectManager->get(CategoryUrlRewriteGenerator::class);
        $urlRewriteBunchReplacer = $this->objectManager->get(UrlRewriteBunchReplacer::class);
        $urlRewriteBunchReplacer->doBunchReplace(
            $categoryUrlRewriteGenerator->generate($ancestorCategory, false, (int)$ancestorCategory->getId())
        );

        $customRewriteCollection = $this->urlRewriteCollectionFactory->create();
        $customRewriteCollection
            ->addFieldToFilter(UrlRewrite::REQUEST_PATH, $requestPath)
            ->addFieldToFilter(UrlRewrite::STORE_ID, $storeId)
            ->addFieldToFilter(UrlRewrite::ENTITY_TYPE, 'custom');
        $customRewriteItem = $customRewriteCollection->getFirstItem();

        $this->assertSame(1, $customRewriteCollection->getSize());
        $this->assertSame('about-us', $customRewriteItem->getTargetPath());

        $descendantCategoryRewriteCollection = $this->getCategoryRewriteCollection($descendantCategoryId);
        $descendantCategoryRewriteCollection->addFieldToFilter(UrlRewrite::REQUEST_PATH, $requestPath);
        $this->assertSame(0, $descendantCategoryRewriteCollection->getSize());
    }
     ...
     * @return void
     */
    public function testCategoryUrlKeyChangeConflictsWithCustomUrlRewrite(): void
    {
        $storeId = 1;
        $requestPath = 'category-1/category-1-1/custom-category.html';
        $urlPersist = $this->objectManager->get(UrlPersistInterface::class);
        $customRewrite = $this->objectManager->get(UrlRewriteFactory::class)->create()
            ->setEntityType('custom')
            ->setEntityId(0)
        ...
        $category = $this->categoryRepository->get(402, $storeId);
